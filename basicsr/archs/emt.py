...
from basicsr.archs.utils import MeanShift, Conv2d1x1, Conv2d3x3, ShiftConv2d1x1, PixelMixer, \
...
class Mlp(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: int = None,
        ...
        self.fc1 = Conv2d1x1(in_features, hidden_features, bias=True)
        self.act = act_layer()
        self.fc2 = Conv2d1x1(hidden_features, out_features, bias=True)
...
class MixedTransformerBlock(TransformerGroup):
    def __init__(self, wn,dim: int, num_layer: int, num_heads: int, num_GTLs: int,
                 window_list: tuple = None, shift_list: tuple = None,
                 mlp_ratio: int = None, act_layer: nn.Module = nn.GELU) -> None:
        token_mixer_list = [
            # TokenMixer(dim) if _ > (num_GTLs - 1) else 
                            SWSA(dim=dim, num_heads=num_heads,attn_layer=[Conv2d1x1(dim, dim * 2),
        ...
        mlp_list = [
            # Mlp(dim, dim * mlp_ratio, act_layer=act_layer)
                    Gated_Conv_FeedForward(wn,dim = dim, dropout = 0.0)
                    for _ in range(num_layer)]
