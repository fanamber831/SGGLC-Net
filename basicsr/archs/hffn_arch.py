...
class AVGC(nn.Module):
    def __init__(self, channel, reduction=16):
        super(AVGC, self).__init__()
...
class ESA(nn.Module):
    def __init__(self, num_feat=48, conv=nn.Conv2d):
...
class FABlock(nn.Module):
    def __init__(self, dim, reduction=16,kernel_size=3 ):
        super(FABlock, self).__init__()
        self.conv1 = conv(dim, dim, kernel_size, bias=True)
        self.act1 = nn.ReLU(inplace=True)
        self.conv2 = conv(dim, dim, kernel_size, bias=True)
        self.calayer = CCAL(dim,reduction=reduction)
        self.palayer = PALayer(dim,reduction=reduction)
...
class HFFBlock(nn.Module):
    def __init__(self,wn,n_feat):
        ...
        self.relu=nn.ReLU(inplace=True)
        self.cca=CCALayer(self.channel,reduction=4)
...
class HFFB(nn.Module):
    def __init__(self,wn,n_feat):
        ...
        self.cca=CCA(self.channels_3,reduction=4)
        # 高频分支
...
class HFFNET(nn.Module):
    def __init__(self, num_in_ch=3,n_feats=48,upscale=2,conv= default_conv):
        ...
        modules_tail = [
            ...
            conv(n_feat, scale * scale * 3, kernel_size=3),
