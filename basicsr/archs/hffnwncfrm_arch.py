...
from basicsr.archs.sfrm import *
...
class HFFNETwncfrm(nn.Module):
    def __init__(self, num_in_ch=3,n_feats=48,upscale=2,conv= default_conv):
        ...
        self.seuqence_7 = []
        for i in range(3):
            self.seuqence_3 += [PCBActiv(wn,n_feat, n_feat, innorm=True)]
            self.seuqence_5 += [PCBActiv(wn,n_feat, n_feat, sample='same-5', innorm=True)]
            self.seuqence_7 += [PCBActiv(wn,n_feat, n_feat, sample='same-7', innorm=True)]

        self.cov_3 = nn.Sequential(*self.seuqence_3)
        self.cov_5 = nn.Sequential(*self.seuqence_5)
        self.cov_7 = nn.Sequential(*self.seuqence_7)

        self.down_fus_DE = wn(nn.Conv2d(n_feat*3, n_feat, 1, 1))

        self.fea1 = FeaBlock(wn,n_feat, n_feat)
    ...
    def forward(self,x,vf):
        ...
        # vf=self.fab(vf)
        #sfrm
        x_DE_3 = self.cov_3(vf)  # torch.Size([8, 128, 112, 112])
        x_DE_5 = self.cov_5(vf)  # torch.Size([8, 128, 112, 112])
        x_DE_7 = self.cov_7(vf)  # torch.Size([8, 128, 112, 112])
        x_DE_fuse = torch.cat([x_DE_3, x_DE_5, x_DE_7], 1) # torch.Size([8, 384, 112, 112])
        x_DE_fi = self.down_fus_DE(x_DE_fuse) 

        vf = self.fea1(x_DE_fi)  
