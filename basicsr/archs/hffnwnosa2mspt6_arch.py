...
class HFFNETwnosamspt6(nn.Module):
    def __init__(self, num_in_ch=3,n_feats=48,upscale=2,conv= default_conv):
        super(HFFNETwnosamspt6, self).__init__()
        ...
        self.conv7=wn(nn.Conv2d(n_feat*2,n_feat,1))
        self.conv8=wn(nn.Conv2d(n_feat*2,n_feat,1))
    ...
    def forward(self,x,vf):
        ...
        p1=self.fab(y1)     
        l1=self.lffb1(p1)
        lv1=self.fab(self.conv4(torch.cat((l1,vf),dim=1)))
        p2=p1+self.fab(p1)
        f1=p2*lv1+lv1
        l2=self.lffb2(f1)
        lv2=self.fab(self.conv5(torch.cat((l2,vf),dim=1)))
        p3=p2+self.fab(p2)
        f2=p3*lv2+lv2
        l3=self.lffb3(f2)
        lv3=self.fab(self.conv6(torch.cat((l3,vf),dim=1)))
        p4=p3+self.fab(p3)
        f3=p4*lv3+lv3
        l4=self.lffb4(f3)
        lv4=self.fab(self.conv7(torch.cat((l4,vf),dim=1)))
        p5=p4+self.fab(p4)
        f4=p5*lv4+lv4
        l5=self.lffb5(f4)
        lv5=self.fab(self.conv8(torch.cat((l5,vf),dim=1)))
        p6=p5+self.fab(p5)
        f5=p6*lv5+lv5
        l6=self.lffb6(f5)
