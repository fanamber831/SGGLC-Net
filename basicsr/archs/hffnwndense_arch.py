...
class LFFB(nn.Module):
    def __init__(self,wn,n_feat):
        ...
        self.c1=wn(nn.Conv2d(n_feat*2,n_feat,1,stride=1,padding=0,groups=2))
        self.c2=wn(nn.Conv2d(n_feat*3,n_feat,1,stride=1,padding=0,groups=2))
        self.c3=wn(nn.Conv2d(n_feat*4,n_feat,1,stride=1,padding=0,groups=1))
    ...
    def forward(self,x):
        h1=self.hffb1(x)
        dense1=torch.cat([x,h1],dim=1)
        # h2=self.hffb2(h1)
        # h3=self.hffb3(h2)
        # h=torch.cat([h1,h2,h3],dim=1)
        # out=self.conv1(h)
        h2=self.hffb2(self.c1(dense1))
        dense2=torch.cat([x,h1,h2],dim=1)
        h3=self.hffb3(self.c2(dense2))
        dense3=torch.cat([x,h1,h2,h3],dim=1)
        out=self.c3(dense3)       
        out=self.relu(out)
