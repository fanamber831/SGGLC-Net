...
class HFFNETwn(nn.Module):
    def __init__(self, num_in_ch=3,n_feats=48,upscale=2,conv= default_conv):
        ...
        self.seuqence_7 = []
        for i in range(3):
            self.seuqence_3 += [PCBActiv(n_feat, n_feat, innorm=True)]
            self.seuqence_5 += [PCBActiv(n_feat, n_feat, sample='same-5', innorm=True)]
            self.seuqence_7 += [PCBActiv(n_feat, n_feat, sample='same-7', innorm=True)]

        self.cov_3 = nn.Sequential(*self.seuqence_3)
        self.cov_5 = nn.Sequential(*self.seuqence_5)
        self.cov_7 = nn.Sequential(*self.seuqence_7)

        self.down_fus_DE = ConvDown(n_feat*3, n_feat, 1, 1)

        self.fea1 = FeaBlock(n_feat, n_feat)
